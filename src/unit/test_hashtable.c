...
int test_scan(int argc, char **argv, int flags) {
    ...
    for (int round = 0; round < num_rounds; round++) {
        ...
        /* Verify every entry was returned at least once, but no more than
         * twice. Entries can be returned twice due to probing chains wrapping
        ...
        printf("max emitted per call: %ld; ", max_entries_per_cycle);
