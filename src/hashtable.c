...
static_assert(sizeof(hashtablePosition) >= sizeof(struct position),
              "Opaque iterator size");
...
static inline struct position *positionFromOpaque(hashtablePosition *p) {
    return (struct position *)(void *)p;
}
...
int hashtableFindPositionForInsert(hashtable *ht, void *key, hashtablePosition *position, void **existing) {
    struct position *p = positionFromOpaque(position);
    uint64_t hash = hashKey(ht, key);
 ...
 * up in the table again. The function returns a pointer to the entry the entry
 * pointer within the hash table, if an entry with a matching key is found, and
 * NULL otherwise.
 *
...
int hashtableIncrementalFindStep(hashtableIncrementalFindState *state) {
    ...
    case HASHTABLE_NEXT_ENTRY:
        ...
        if (data->bucket->presence != 0 && data->pos < numBucketPoitions(data->bucket)) {
            bucket *b = data->bucket;
            uint8_t h2 = highBits(data->hash);
            for (int pos = data->pos; pos < numBucketPoitions(b); pos++) {
                if (isPositionFilled(b, pos) && b->hashes[pos] == h2) {
                    /* It's a candidate. */
